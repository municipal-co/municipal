...
export function SnapScrollSlider({className, children, settings}) {
  const slider = useRef();
  const slides = useRef([]);
  ...
  const observerCallback = (entries) => {
    entries.forEach(entry => {
      ...
      if(internalSettings.enableArrows && prevArrow.current !== null) {
        if( slides.current[0].className.indexOf('is-visible') > -1) {
      ...
      if(internalSettings.enableArrows &&  nextArrow.current !== null) {
        if(slides.current[slides.current.length - 1].className.indexOf('is-visible') > -1) {
  ...
  const goToNextSlide = () => {
    ...
    if(slideIndex < slides.current.length - 1) {
      const cardCoordinate = slides.current[slideIndex + 1].offsetLeft - Number.parseInt(window.getComputedStyle(slider.current).scrollPadding.replace('0px ', '').replace('px', '') * 2);
  ...
  const goToPrevSlide = () => {
    ...
    if(slideIndex > 0) {
      ...
      const cardCoordinate = firstInactiveSlide.offsetLeft - Number.parseInt(window.getComputedStyle(slider.current).scrollPadding.replace('0px ', '').replace('px', '') * 2);
      slider.current.scrollTo({
        left: cardCoordinate,
        behavior: 'smooth'
      })
  ...
  return(
    <>
      ...
      <div className={`snap-slider-wrapper ${className}`}
      style={{
